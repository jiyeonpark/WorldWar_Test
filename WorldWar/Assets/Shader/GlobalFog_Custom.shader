...
Shader "Hidden/GlobalFog_Custom" {
Properties {
	_MainTex ("Base (RGB)", 2D) = "black" {}
	_FogColor("Color", Color) = (1,1,1,1)	// CustomCode!
	_FogFlag("Fog Flag", Range(0.0, 1.0)) = 0.5	// CustomCode!
}
...
CGINCLUDE
	...
	#endif	
	half4 _FogColor;	// CustomCode!
	half _FogFlag;	// CustomCode!
	...
	half4 ComputeFog (v2f i, bool distance, bool height) : SV_Target
	{
		...
		_FogColor = _FogColor * _FogFlag;
		fogFac = lerp (fogFac, 1.0, _FogFlag);
		half4 fogcolor = lerp (_FogColor, sceneColor, fogFac);
		return fogcolor;
		// CustomCode!
	}
